...
from scriptspy import inventar, icons, support, button, explosions, functions, loading, shoots, character, heart, boss_game, opponents, meteor, achievment, cursors
...
def game():
    ...
    cursor = cursors.Cursor(skin_cursor)
...
def preview():
    ...
    functions.newplayer()
    pygame.mouse.set_visible(False)
    global all_sprites, drobs, bullets, vaders_group, bosses, cursor_group
...
def settings():
    ...
    pygame.display.set_mode((x, y - 27), pygame.HWSURFACE | pygame.DOUBLEBUF)
    ...
    def destroy():
        pygame.display.set_mode((x, y - 50), pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.FULLSCREEN)
        speed()
        skins()
        reset()
        root.destroy()

    img_ram2 = ImageTk.PhotoImage(loading.img_ram)
    ...
    img_tk_aim1 = ImageTk.PhotoImage(loading.img_tk_aim1)
    img_tk_aim2 = ImageTk.PhotoImage(loading.img_tk_aim2)
    img_tk_aim3 = ImageTk.PhotoImage(loading.img_tk_aim3)
    img_tk_aim4 = ImageTk.PhotoImage(loading.img_tk_aim4)
    img_tk_aim5 = ImageTk.PhotoImage(loading.img_tk_aim5)
    img_tk_aim6 = ImageTk.PhotoImage(loading.img_tk_aim6)
    img_tk_aim7 = ImageTk.PhotoImage(loading.img_tk_aim7)
    img_tk_aim8 = ImageTk.PhotoImage(loading.img_tk_aim8)

    canvas = Canvas(root, width=x+10, height=y+10, bg='black', bd=0, highlightthickness=0)
    canvas.pack()

    canvas.create_image(x//2, y//2-25, image=img_ram2)

    text = canvas.create_text(270, 20, font=("Comic Sans MS", 15), text="Минимальная скорость врагов(по умолчанию 4)",
                              fill="#dceca4")
    scale1 = Scale(canvas, orient=HORIZONTAL, length=700, from_=1, to=25, tickinterval=1, resolution=1, bg='black',
                   fg='#dceca4', bd=0, highlightthickness=0)
    ...
    if sprites:
        button_sprites = Button(text="Да", command=sprite, bd=0, highlightthickness=0)
        ...
        button_sprites = Button(root, text='Нет', command=sprite, bd=0, highlightthickness=0)
        button_sprites.place(x=350, y=130)
        button_sprites['bg'] = '#dceca4'
        button_sprites['activebackground'] = 'black'
        button_sprites['fg'] = 'black'
    ...
    canvas.create_image(60, 275, image=img_vader4)
    canvas.create_image(148, 275, image=img_putin)
    canvas.create_image(238, 275, image=img_virus)
    canvas.create_image(326, 275, image=img_vaders)
    if achievment.gameovers >= 50:
        canvas.create_image(420, 275, image=img_skull2)
    var = IntVar()
    var.set(3)
    radio = Radiobutton(root, variable=var, value=0)
    radio.place(x=45, y=320)
    radio = Radiobutton(root, variable=var, value=1)
    radio.place(x=135, y=320)
    radio = Radiobutton(root, variable=var, value=2)
    radio.place(x=225, y=320)
    radio = Radiobutton(root, variable=var, value=3)
    radio.place(x=315, y=320)
    if achievment.gameovers >= 50:
        radio = Radiobutton(root, variable=var, value=4)
        radio.place(x=404, y=320)
    ...
    text = canvas.create_text(270, 610, font=("Comic Sans MS", 15), text="Здесь можно посмотреть текущее управление",
                              fill="#dceca4")
    canvas.create_image(17, 610, image=img_option2)
    button = Button(text="Здесь", command=moving, font=("Comic Sans MS", 10), bd=0, highlightthickness=0)
    ...
    text = canvas.create_text(700, 230, font=("Comic Sans MS", 15), text="Изображения для курсора", fill="#dceca4")
    canvas.create_image(557, 230, image=img_option2)
    button1 = Button(command=skins1, image=img_tk_aim1, bd=0, highlightthickness=0)
    button1.place(x=525, y=265)
    button1 = Button(command=skins2, image=img_tk_aim2, bd=0, highlightthickness=0)
    button1.place(x=595, y=265)
    button1 = Button(command=skins3, image=img_tk_aim3, bd=0, highlightthickness=0)
    button1.place(x=665, y=265)
    button1 = Button(command=skins4, image=img_tk_aim4, bd=0, highlightthickness=0)
    button1.place(x=735, y=265)
    button1 = Button(command=skins5, image=img_tk_aim5, bd=0, highlightthickness=0)
    button1.place(x=805, y=265)
    button1 = Button(command=skins6, image=img_tk_aim6, bd=0, highlightthickness=0)
    button1.place(x=595, y=335)
    button1 = Button(command=skins7, image=img_tk_aim7, bd=0, highlightthickness=0)
    button1.place(x=665, y=335)
    button1 = Button(command=skins8, image=img_tk_aim8, bd=0, highlightthickness=0)
    button1.place(x=735, y=335)

    d = Button(text="Подтведить эти изменения", command=destroy, font=("Comic Sans MS", 14), bd=0,
...
def translator_google(text, who, suffer):
    interpreter = Translator(from_lang=who, to_lang=suffer)
    dragoman = interpreter.translate(text)
    return dragoman
...
def reset():
    global vaders_group, all_sprites, cursor_group
    for i in vaders_group:
        i.kill()
    for i in range(25):
        imp = opponents.Imperia(jelly, putin, virus, vaders, death, sprites, bosses,
...
# print(translator_google('Опция', 'ru', 'en'))
windows()
preview()
