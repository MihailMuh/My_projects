...
def achievements():
    pygame.display.set_mode((system_size.x, system_size.y - 27), pygame.HWSURFACE | pygame.DOUBLEBUF)

    root = Tk()
    root.geometry("{0}x{1}+0+0".format(root.winfo_screenwidth()+10, root.winfo_screenheight()+10))

    def break_root():
        pygame.display.set_mode((system_size.x, system_size.y - 50), pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.FULLSCREEN)
